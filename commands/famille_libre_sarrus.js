...
const { generateFamilyNotLinearlyDependent, generateFamilyLinearlyDependent } = require("../utils/family");
...
module.exports = {
    ...
    async run(bot, interaction) {
        const { family } = Math.random() < 0.5 ? generateFamilyLinearlyDependent() : generateFamilyNotLinearlyDependent();
        ...
        const statement = `**Détermination de la liberté d'une famille avec Sarrus**\n\n` +
        ...
        \\textbf{${family[0][0]}} & \\textbf{{${family[0][1]}}} & \\textbf{{${family[0][2]}}} \\\\
        \\textbf{${family[1][0]}} & \\textbf{${family[1][1]}} & \\textbf{{${family[1][2]}}} \\\\
        \\end{vmatrix} \\\\\\\\
        ...
        {\\color{DarkGreen} ${family[1][0]}} & {\\color{Red} ${family[1][1]}} & ${family[1][2]} \\\\
        {\\color{Blue} ${family[2][0]}} & {\\color{DarkGreen} ${family[2][1]}} & {\\color{Red} ${family[2][2]}} \\\\
        ${family[0][0]} & {\\color{Blue} ${family[0][1]}} & {\\color{DarkGreen} ${family[0][2]}} \\\\
        ...
        \\text{On commence par la \\textbf{diagonale rouge} : (${family[0][0]} × ${family[1][1]} × ${family[2][2]} = ${sumRed})} \\\\
        \\text{On passe ensuite \\\`{a} la \\textbf{diagonale verte} : (${family[1][0]} × ${family[2][1]} × ${family[0][2]} = ${sumGreen})} \\\\
        \\text{Enfin, on passe \\\`{a} la \\textbf{diagonale bleue} : (${family[2][0]} × ${family[0][1]} × ${family[1][2]} = ${sumBlue})} \\\\
        ...
        ${family[1][0]} & {\\color{Red} ${family[1][1]}} & {\\color{DarkGreen} ${family[1][2]}} \\\\
        {\\color{Red} ${family[2][0]}} & {\\color{DarkGreen} ${family[2][1]}} & {\\color{Blue} ${family[2][2]}} \\\\
        {\\color{DarkGreen} ${family[0][0]}} & {\\color{Blue} ${family[0][1]}} & ${family[0][2]} \\\\
        {\\color{Blue} ${family[1][0]}} & ${family[1][1]} & ${family[1][2]} \\\\
        \\end{vmatrix} \\\\\\\\
        
        \\text{On commence par la \\textbf{diagonale rouge} : (${family[0][2]} × ${family[1][1]} × ${family[2][0]} = ${sumRed2})} \\\\
        \\text{On passe ensuite \\\`{a} la \\textbf{diagonale verte} : (${family[1][2]} × ${family[2][1]} × ${family[0][0]} = ${sumGreen2})} \\\\
        \\text{Enfin, on passe \\\`{a} la \\textbf{diagonale bleue} : (${family[2][2]} × ${family[0][1]} × ${family[1][0]} = ${sumBlue2})} \\\\
        ...
        ${sum - sum2 === 0 ? `\\text{Le déterminant est nul, donc la famille n'est pas libre.}` : `\\text{Le déterminant est non nul, donc la famille est libre.}`} \\\\\\\\
