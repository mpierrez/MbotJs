...
module.exports = {
  ...
  description:
    "Vérifier si un vecteur est une combinaison linéaire de trois autres vecteurs",
  category: "algebre",
  ...
  async run(bot, interaction) {
    ...
    const v1 = {
      x: system.coefficients[0][0],
      y: system.coefficients[1][0],
      z: system.coefficients[2][0],
    };
    const v2 = {
      x: system.coefficients[0][1],
      y: system.coefficients[1][1],
      z: system.coefficients[2][1],
    };
    const v3 = {
      x: system.coefficients[0][2],
      y: system.coefficients[1][2],
      z: system.coefficients[2][2],
    };
    const w = {
      x: system.results[0],
      y: system.results[1],
    ...
    const statement =
      `**Détermination d'une combinaison linéaire**\n\n` +
    ...
    const latexResult = `\\begin{array}{c}
        ...
        ${
          isResolvable
            ? `\\text{Dans notre cas, les valeurs de x, y et z sont :} \\\\
                            x = ${x} \\\\
                            y = ${y} \\\\
                            z = ${z}`
        ...
        ${
          isResolvable
            ? `\\text{Donc w est une combinaison lineaire des vecteurs v1, v2, v3.} \\\\\\\\`
            : `\\text{Donc w n est pas une combinaison lineaire des vecteurs v1, v2, v3.}`
        }
        
        ${
          isResolvable
            ? `w = ${formatTerm(x, v1, "Red")} ${
                x !== 0 && y !== 0 ? "+" : ""
              } ${formatTerm(y, v2, "DarkGreen")} ${
                (x !== 0 || y !== 0) && z !== 0 ? "+" : ""
              } ${formatTerm(z, v3, "Blue")} \\\\\\\\`
            : ""
        }
