...
module.exports = {
  name: "mineur3x3",
  description:
    "Déterminer le mineur d'une matrice 3x3 et calculer son déterminant",
  category: "algebre",
  ...
  async run(bot, interaction) {
    ...
    const explanation = `\\begin{array}{c}
        ...
        \\begin{bmatrix}
            ${
              i === 1 || j === 1
                ? `{\\color{Red}${matrix3x3[0][0]}}`
                : matrix3x3[0][0]
            } & ${
      i === 1 || j === 2 ? `{\\color{Red}${matrix3x3[0][1]}}` : matrix3x3[0][1]
    } & ${
      i === 1 || j === 3 ? `{\\color{Red}${matrix3x3[0][2]}}` : matrix3x3[0][2]
    } \\\\
            ${
              i === 2 || j === 1
                ? `{\\color{Red}${matrix3x3[1][0]}}`
                : matrix3x3[1][0]
            } & ${
      i === 2 || j === 2 ? `{\\color{Red}${matrix3x3[1][1]}}` : matrix3x3[1][1]
    } & ${
      i === 2 || j === 3 ? `{\\color{Red}${matrix3x3[1][2]}}` : matrix3x3[1][2]
    } \\\\
            ${
              i === 3 || j === 1
                ? `{\\color{Red}${matrix3x3[2][0]}}`
                : matrix3x3[2][0]
            } & ${
      i === 3 || j === 2 ? `{\\color{Red}${matrix3x3[2][1]}}` : matrix3x3[2][1]
    } & ${
      i === 3 || j === 3 ? `{\\color{Red}${matrix3x3[2][2]}}` : matrix3x3[2][2]
    } \\\\
        \\end{bmatrix} \\\\\\\\
        ...
        \\text{Dans notre cas : } a_{1,1} = ${
          minorMatrix[0][0]
        }\\text{, } a_{2,2} = ${minorMatrix[1][1]}\\text{, } a_{1,2} = ${
      minorMatrix[0][1]
    }\\text{ et } a_{2,1} = ${minorMatrix[1][0]} \\\\
        \\text{Il ne reste plus qu à remplacer dans la formule :} \\\\
        D = ${minorMatrix[0][0]} * ${minorMatrix[1][1]} - ${
      minorMatrix[0][1]
    } * ${minorMatrix[1][0]} = ${minorDet} \\\\\\\\
