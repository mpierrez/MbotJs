...
module.exports = {
  name: "syst_cramer",
  description:
    "Résoudre un système d'équations à 3 inconnues en utilisant la méthode de Cramer",
  category: "algebre",
  ...
  async run(bot, interaction) {
    ...
    const statement =
      `**Déterminant d'une matrice 3x3 avec la méthode de Cramer**\n\n` +
    ...
    const latexResult = `\\begin{array}{c}
        ...
        ${matrixToLatex(system.coefficients)} ${matrixToLatex(
      system.results
    )} \\\\\\\\
        ...
        D = 
        \\begin{vmatrix}
        {${system.coefficients[0][0]}} & ${system.coefficients[0][1]} & ${
      system.coefficients[0][2]
    } \\\\
        {${system.coefficients[1][0]}} & {${system.coefficients[1][1]}} & ${
      system.coefficients[1][2]
    } \\\\
        {${system.coefficients[2][0]}} & {${system.coefficients[2][1]}} & {${
      system.coefficients[2][2]
    }} \\\\
        \\end{vmatrix} = ${det}\\\\\\\\
        ...
        X = 
        \\begin{pmatrix}
        {\\color{Red}${system.results[0]}} & ${system.coefficients[0][1]} & ${
      system.coefficients[0][2]
    } \\\\
        {\\color{Red}${system.results[1]}} & ${system.coefficients[1][1]} & ${
      system.coefficients[1][2]
    } \\\\
        {\\color{Red}${system.results[2]}} & ${system.coefficients[2][1]} & ${
      system.coefficients[2][2]
    } \\\\
        \\end{pmatrix} \\\\\\\\
        ...
        Y =
        \\begin{pmatrix}
        ${system.coefficients[0][0]} & {\\color{Red}${system.results[0]}} & ${
      system.coefficients[0][2]
    } \\\\
        ${system.coefficients[1][0]} & {\\color{Red}${system.results[1]}} & ${
      system.coefficients[1][2]
    } \\\\
        ${system.coefficients[2][0]} & {\\color{Red}${system.results[2]}} & ${
      system.coefficients[2][2]
    } \\\\
        \\end{vmatrix} \\\\\\\\
      ...
      system.coefficients[2][1]
        ...
        D_{x} = 
        \\begin{vmatrix}
        {${system.results[0]}} & ${system.coefficients[0][1]} & ${
      system.coefficients[0][2]
    } \\\\
        {${system.results[1]}} & {${system.coefficients[1][1]}} & ${
      system.coefficients[1][2]
    } \\\\
        {${system.results[2]}} & {${system.coefficients[2][1]}} & {${
      system.coefficients[2][2]
    }} \\\\
        \\end{vmatrix} = ${detX} \\\\\\\\
        ...
        D_{y} =
        \\begin{vmatrix}
        {${system.coefficients[0][0]}} & {${system.results[0]}} & ${
      system.coefficients[0][2]
    } \\\\
        {${system.coefficients[1][0]}} & {${system.results[1]}} & ${
      system.coefficients[1][2]
    } \\\\
        {${system.coefficients[2][0]}} & {${system.results[2]}} & {${
      system.coefficients[2][2]
    }} \\\\
        \\end{vmatrix} = ${detY} \\\\\\\\
    ...
    const attachmentStatement = new AttachmentBuilder(
      `https://latex.codecogs.com/png.latex?\\dpi{300}&space;\\bg_white&space;\\begin{align}${encodeURIComponent(
        systemToLatex(system.coefficients, system.results)
      )}\\end{align}`,
      { name: "equations.png" }
    );
