...
const {
  generateObviousFamilyLinearlyDependent,
...
module.exports = {
  ...
  async run(bot, interaction) {
    const { family, explanation, explanation2, explanation3 } =
      Math.random() < 0.75
        ? generateObviousFamilyLinearlyDependent()
        : generateFamilyNotLinearlyDependent();
    ...
    const statement =
      `**Détermination de la liberté d'une famille**\n\n` +
    ...
    const latexResult = `\\begin{array}{c}
        ...
        {\\color{Red}v1 = (${family[0][0]}, ${family[0][1]}, ${
      family[0][2]
    })} \\\\
        {\\color{DarkGreen}v2 = (${family[1][0]}, ${family[1][1]}, ${
      family[1][2]
    })} \\\\
        {\\color{Blue}v3 = (${family[2][0]}, ${family[2][1]}, ${
      family[2][2]
    }}) \\\\\\\\

        ${
          explanation != null
            ? `\\text{La famille est liee car ${explanation}} \\\\\\\\`
