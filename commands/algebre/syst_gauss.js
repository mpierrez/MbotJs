...
const { generateSystem, pivotGauss } = require("../../utils/system");

module.exports = {
  name: "syst_gauss",
  description:
    "Résoudre un système d'équations à 3 inconnues avec la méthode du pivot de Gauss",
  category: "algebre",
  ...
  async run(bot, interaction) {
    ...
    const statement =
      `**Déterminant d'une matrice 3x3 avec la méthode du pivot de Gauss**\n\n` +
      `Résolvez le système suivant en utilisant la méthode du pivot de Gauss :\n`;
    ...
    const resultExplanation = `\\begin{array}{c}
        ...
        ${gaussSystemToLatex(system.coefficients, system.results, [
          { i: 0, j: 0 },
        ])} \\\\\\\\
        ...
        \\textbf{Normalisation}
        ${gaussSystemToLatex(
          step1.map((row) => row.slice(0, 3)),
          step1.map((row) => row[3]),
          [],
          [{ i: 1, j: 0 }],
          [{ i: 2, j: 0 }]
        )} \\\\\\\\
        \\text{Ce que j ai fait ici, c'est diviser toute la premi\\\`{e}re ligne par } {\\color{Red}a_{11} = ${
          system.coefficients[0][0]
        }} \\text{ pour que le } \\textbf{pivot soit \\'{e}gal \\\`{a} 1}. \\\\\\\\
        \\text{==============================================}\\\\\\\\

        \\textbf{Elimination}
        ${gaussSystemToLatex(
          step2.map((row) => row.slice(0, 3)),
          step2.map((row) => row[3]),
          [{ i: 1, j: 1 }]
        )} \\\\\\\\
        ...
        \\textbf{Normalisation}
        ${gaussSystemToLatex(
          step3.map((row) => row.slice(0, 3)),
          step3.map((row) => row[3]),
          [],
          [{ i: 0, j: 1 }],
          [{ i: 2, j: 1 }]
        )} \\\\\\\\
        \\text{Ce que j ai fait ici, c'est diviser toute la deuxi\\\`{e}me ligne par } {\\color{Red}a_{22} = ${
          step2[1][1]
        }} \\text{ pour que le } \\textbf{pivot soit \\'{e}gal \\\`{a} 1}. \\\\\\\\
        \\text{==============================================}\\\\\\\\
        ...
        \\textbf{Normalisation}
        ${gaussSystemToLatex(
          step5.map((row) => row.slice(0, 3)),
          step5.map((row) => row[3]),
          [],
          [{ i: 0, j: 2 }],
          [{ i: 1, j: 2 }]
        )} \\\\\\\\
        \\text{Ce que j ai fait ici, c'est diviser toute la troisi\\\`{e}me ligne par } {\\color{Red}a_{33} = ${
          step4[2][2]
        }} \\text{ pour que le } \\textbf{pivot soit \\'{e}gal \\\`{a} 1}. \\\\\\\\
        \\text{==============================================}\\\\\\\\
